...
데이터 컨텍스트란 일종의 경로라고 볼 수 있습니다. 콘솔 창에서 명령어를 입력할 때 현재 경로에 있는 파일을 참조할 때는 그냥 파일명만 입력할 수 있지만 다른 폴더에 있는 파일을 참조할 때는 절대 경로를 사용하게 됩니다. 데이터 컨텍스트는 XAML에서 현재 경로를 지정해주는 역할을 한다고 보면됩니다. 경로를 지정하면 Binding 구문에서 해당 경로의 프로퍼티에 접근할 수 있습니다.

상당히 복잡한 듯 보이지만 이해하고 나면 쉽습니다. 다만 Android의 데이터바인딩과는 다르게 직관적이지 않다는 점이 흠입니다.  
